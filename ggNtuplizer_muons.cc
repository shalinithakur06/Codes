...
void ggNtuplizer::branchesMuons(TTree* tree) {
  ...
  tree->Branch("trigDecision",           &trigDecision);
  ...
  tree->Branch("muIsGlob",      &muIsGlob_);
  tree->Branch("muIsPF",        &muIsPF_);
  tree->Branch("muIsTrk",       &muIsTrk_);
...
//
   ...
   e.getByToken(trgResultsLabel_, triggerBits);
  ...
  trigDecision.insert(trigDecision.begin()+2,0);
   ...
   for(unsigned i=0;i<names.size();i++){
        if((names.triggerName(i).find("HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_v") != npos) || (names.triggerName(i).find("HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ_v") != npos)){
	  if(triggerBits->accept(i)){
            trigDecision.insert(trigDecision.begin()+0,1);
//cout << "trig 0 " << trigDecision.size() <<endl;
	  }
	}

        if(names.triggerName(i).find("HLT_IsoMu20_v") != npos){
          if(triggerBits->accept(i)){
             trigDecision.insert(trigDecision.begin()+1,1);
//cout << "trig 1 " << endl;
       }} 

       if(names.triggerName(i).find("HLT_Mu17_v") != npos){
          if(triggerBits->accept(i)){
             trigDecision.insert(trigDecision.begin()+2,1);
//cout << "trig 2 " << trigDecision.size() << endl;
       }}
   
     }/*Trigger Loop*/
//cout << "trigDecision size " << trigDecision.size() << endl;
//for(unsigned int i=0; i < trigDecision.size(); ++i) cout << "i " << i << "\t" << trigDecision[i] << endl;
//
  ...
  for (edm::View<pat::Muon>::const_iterator iMu = muonHandle->begin(); iMu != muonHandle->end(); ++iMu) {
    ...
    muIsGlob_  .push_back(iMu->isGlobalMuon());
    muIsPF_  .push_back(iMu->isPFMuon());
    muIsTrk_  .push_back(iMu->isTrackerMuon());
...
//std::cout << "Global/PF/Tracker/type/IDbit " << iMu->isGlobalMuon() << "\t" << iMu->isPFMuon() << "\t" << iMu->isTrackerMuon()  << "\t" << iMu->type() << "\t" << tmpmuIDbit << endl;
