...
{
  if (fChain == 0) return;
  TFile *f = new TFile("ntuples_DYmad_2017_10_21.root", "RECREATE");
  ...
  tree->Branch("GenWeight", &GenWeight, "GenWeight");
  tree->Branch("mc_EventWeight", &mc_EventWeight, "mc_EventWeight");
  ...
  TH1F *nGVtx = new TH1F("nGVtx", "no. of good primary vertices", 75, 0,75);
  ...
  TH1D *npu = new TH1D("npu" , "no. of PU" , 75, 0, 75);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    nvtx->Fill(nVtxs);
    int nPU_size = nPU->size();
    for (int ipu=0; ipu < nPU_size; ipu++){ 
    npu->Fill((*nPU)[ipu]);
    }
    //npu->Fill(nPUs);
    ...
    //nvtx->Fill(nGoodVtx);
    nGVtx->Fill(nGVtxs);
    //cout << " nvtx = " << nVtxs << " nGVtxs = " << nGVtxs << endl;
